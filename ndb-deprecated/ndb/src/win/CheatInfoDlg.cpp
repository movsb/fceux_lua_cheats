...
#define __FCEUX__
			...
			if(!hFCEUX){
				MessageBox("请先打开FCEUX",NULL,MB_ICONEXCLAMATION);
				return 0;
			}
#else
...
void ACheatInfoDlg::MoveItemToAnotherPlace(int ifrom,int ito)
{
	...
	list_remove(&m_pEntry->item.values,pfrom); //移除起始节点并插入到终止节点后
	...
	ito++;					//插入到ito的后面,所以++
	if(ifrom>ito) {}
	if(ifrom<ito) {ito--;}	//ifrom<ito,ifrom被移除,位置就是ito+1-1
